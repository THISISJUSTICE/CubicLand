...
public abstract class DVCubeBase : MonoBehaviour
{
    ...
    public DVCubeInfo CubeInfo { get => _cubeInfo; }
    ...
    public virtual void SetCubeInfo(DVCubeInfo cubeInfo) {
        _cubeInfo = cubeInfo;
        SetCubeShader();
        CubeMass = DVUtil.GetCubeMass(cubeInfo.CurrentStatus);
    }

    public void OnDamaged(float selfMass, Vector3 impulse, DVCubeInfo? colCubeInfo = null)
    {
    ...
    public virtual void OnDamaged(float selfMass, Vector3 impulse, DVCubeInfo? colCubeInfo, out float damageRate)
    {
        ...
        if (colCubeInfo != null)
            _cubeInfo.CurrentStatus.OnDamaged(selfMass, impulse, (DVCubeInfo)colCubeInfo);
        else
        ...
        {
            CubeMass = DVUtil.GetCubeMass(_cubeInfo.CurrentStatus);
        }
    }
    #endregion
