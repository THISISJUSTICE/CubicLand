...
namespace CustomTIJI.CubicLand
{
    public class GolemController : MonoBehaviour
    {
            ...
            public Vector3 GetDirection(Enums.Direction3D direction)
            {
                switch (direction)
                {
                    ...
                    case Enums.Direction3D.Right:
                        return GetDirection(Utils.ConvertDirection3DTo2D(direction));
        ...
        protected GolemCore _golemCore;
        ...
        public GolemCube AxisCube { get; private set; }
        public GolemCube UpEdgeCube { get; private set; }
        public GolemCube BackEdgeCube { get; private set; }
        public GolemCube FrontEdgeCube { get; private set; }
        ...
        protected GolemInfo GolemInfo { get => _golemCore.CurrentGolemInfo; }
        protected int AnimationFrame { get => PerfomanceConfigs.AnimationFrame; }
        ...
        {
            _golemCore = GetComponent<GolemCore>();
        ...
        {
            AxisCube = _golemCore.FindCube(FindRotateAxis(Enums.Direction.Right));
            UpEdgeCube = _golemCore.FindCube(FindAxisCandidates(Enums.Direction3D.Up)[0]);
            BackEdgeCube = _golemCore.FindCube(FindAxisCandidates(Enums.Direction3D.Back)[0]);
            FrontEdgeCube = _golemCore.FindCube(FindAxisCandidates(Enums.Direction3D.Front)[0]);
        ...
        protected void RollGolem(Enums.Direction direction)
        {
            if (_move.Acting)
                return;
            _move.ActFlag = true;
        ...
        protected void MoveJump(Enums.Direction direction)
        {
            if (_move.Acting)
                return;
            _move.ActFlag = true;
        ...
        protected void Rotate(Enums.Direction direction)
        {
            if (_move.Acting)
                return;
            _move.ActFlag = true;
        ...
        #region Utils
        public Enums.Direction3D ConvertMoveToTransformDirection(Enums.Direction3D direction)
        {
            Vector3[] tDirs = transform.GetDirections();
            Vector3 mDir = _moveDirection.GetDirection(direction);

            return Utils.ConvertDirection(tDirs, mDir);
        }

        protected Enums.Direction3D ConvertTransformToMoveDirection(Enums.Direction3D direction)
        {
            Vector3[] mDirs = _moveDirection.GetDirections();
            Vector3 tDir = transform.GetDirections()[(int)direction];

            return Utils.ConvertDirection(mDirs, tDir);
        }
        ...
        protected List<Vector3Int> FindEdgeCubes(Enums.Direction3D moveDirection)
        {
            Enums.Direction3D direction = ConvertMoveToTransformDirection(moveDirection);
            int directionSize = GolemInfo.GetDirectionSize(direction);
            return GolemInfo.FindEdgeChilds(direction, directionSize);
        }

        protected List<Vector3Int> FindAxisCandidates(Enums.Direction3D moveDirection)
        {
        ...
        protected Vector3Int FindRotateAxis(Enums.Direction rotDirection)
        {
            ...
            Dictionary<Enums.Direction3D, Vector3Int> dic = new Dictionary<Enums.Direction3D, Vector3Int>();
            foreach (var cube in nearestCubes)
            {
                Vector3 dir = cube - Vector3Int.zero;
                dir.y = 0f;

                var key = Utils.ConvertDirection(_moveDirection.GetDirections(), dir);
                dic[key] = cube;
            }

            Enums.Direction3D[] priorities = new Enums.Direction3D[] {
            Enums.Direction3D.Front,
            Enums.Direction3D.Right,
            Enums.Direction3D.Back,
            Enums.Direction3D.Left
        };
            if (rotDirection == Enums.Direction.Left)
            {
                priorities = new Enums.Direction3D[] {
                Enums.Direction3D.Back,
                Enums.Direction3D.Left,
                Enums.Direction3D.Right,
                Enums.Direction3D.Front
        ...
        protected float CalculateRotateTime()
        {
            ...
            int bottomCount = FindEdgeCubes(Enums.Direction3D.Down).Count;
        ...
        protected int GetGolemLength(Enums.Direction3D direction)
        {
        ...
        protected IEnumerator MoveJumpCor(Vector3 dir, float time)
        {
            ...
            Vector3 targetPos = (prevPos + dir.normalized * Configs.CUBE_BASE_LENGHT).NormalizeCube();
            ...
            float moveHeight = Configs.CUBE_BASE_LENGHT * 0.5f + startHeight;
            float addHeight;
            ...
            while (_move.ActFlag)
            {
                for (int i = 0; i < AnimationFrame; i++)
                {
                    ...
                    if (!_jumping)
                    {
                        ...
                        {
                            addHeight = Mathf.Lerp(startHeight, moveHeight, Utils.GetEaseOut((float)(i + 1) / (float)halfFrame)) -
                            Mathf.Lerp(startHeight, moveHeight, Utils.GetEaseOut((float)i / (float)halfFrame));
                        }
                        else
                        {
                            halfIndex = i % halfFrame;
                            addHeight = Mathf.Lerp(startHeight, moveHeight, Utils.GetEaseOut((float)(halfFrame - halfIndex - 1) / (float)halfFrame)) -
                            Mathf.Lerp(startHeight, moveHeight, Utils.GetEaseOut((float)(halfFrame - halfIndex) / (float)halfFrame));
                        }
        ...
        protected IEnumerator RollRightAngleCor(Enums.Direction direction, float time)
        {
            _move.Acting = true;
            ...
            float addTime = time / (float)AnimationFrame;
            float halfLine = Configs.CUBE_BASE_LENGHT / 2f;
            ...
            GolemCube prevAxisCube, nextAxisCube;
            GolemCube prevFrontCube, nextFrontCube;

            while (_move.ActFlag)
            {
                curCube = GetGolemLength(Enums.Direction3D.Down);
                if (curCube == 0)
                    curCube = 1;
                nextCube = GetGolemLength(Utils.ConvertDirection2DTo3D(direction));

                prevAxisCube = AxisCube;
                nextAxisCube = _golemCore.FindCube(FindAxisCandidates(Utils.ConvertDirection2DTo3D(direction))[0]);

                prevFrontCube = FrontEdgeCube;
                nextFrontCube = direction == Enums.Direction.Back ?
                    _golemCore.FindCube(FindAxisCandidates(Enums.Direction3D.Down)[0]) : prevFrontCube;

                curHeight = ((float)curCube * 2f + 1f) * halfLine;
                nextHeight = ((float)nextCube * 2f + 1f) * halfLine;
                rollAxisAngle = Utils.GetAngleBH(nextHeight, curHeight);
                rollHypot = Utils.GetHypotenuseBH(nextHeight, curHeight);
                ...
                for (int i = 0; i < AnimationFrame; i++)
                {
                    ...
                    transform.position += Vector3.up *
                        (Utils.GetHeightLineHyA(rollHypot, angle + rollAxisAngle)
                        - Utils.GetHeightLineHyA(rollHypot, prevAngle + rollAxisAngle));

                    // Move
                    transform.position += moveDir *
                        (Utils.GetBaseLineHyA(rollHypot, angle + rollAxisAngle)
                        - Utils.GetBaseLineHyA(rollHypot, prevAngle + rollAxisAngle));
        ...
        protected IEnumerator RotateRightAngleCor(Enums.Direction direction)
        {
            _move.Acting = true;
            _golemCore.SetAttackMode(true);
        ...
        protected IEnumerator ResizeDownCor(float chargeTime)
        {
            ...
            Utils.GetClosestAxisVector(tDirs, Vector3.up, out int index);
            ...
            for (int i = 0; i < MaxJumpHeight * AnimationFrame / 2; i++)
            {
                ...
                yield return Helper.YieldCache.GetWaitForSeconds(stdTime);
            }

            while (true)
                yield return Helper.YieldCache.GetWaitForSeconds(10f);
        }
        ...
        protected IEnumerator KnockbackCor(Vector3 impulse)
        {
            ...
            if (restTime > 0f)
                yield return Helper.YieldCache.GetWaitForSeconds(restTime);
        ...
        protected IEnumerator NormalizeRotationCor(float time)
        {
            ...
            yield return StartCoroutine(Utils.NormalizeRotationCor(transform, prevRot, time));
            PlayerRotation = _moveDirection.GetRotation();
            PlayerViewRotation = PlayerRotation;
        }
        #endregion
