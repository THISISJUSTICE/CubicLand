...
public abstract class DVCubeCore<T> : MonoBehaviour where T : DVCubeBase
{
    #region Variables
    [Header("Viewer")]
    [SerializeField] protected DVGolemInfo _golemInfo;
    [SerializeField] protected T _golemCube;
    [SerializeField] protected DVGolemInfo _curGolemInfo;
    ...
    {
        float weightFactor = Mathf.Pow(_rb.mass, 0.2f) / Mathf.Pow(DVConfigs.INIT_CUBE_MASS, 0.2f);
        float speedFactor = Mathf.Pow((float)DVStatusConfig.INIT_MOVE_SPEED / (float)_golemInfo.MoveSpeed, 0.3f);
    ...
    protected void OnCoreCollision(Collision collision)
    {
        ...
        else if (obstacle != null) // Obstacle Collision
        {
            ...
            {
                Vector3 impulse = DVUtil.EstimateImpulse(MoveVelocity, _rb.mass, obstacle.Velocity, obstacle.CubeMass, normalAVG);
