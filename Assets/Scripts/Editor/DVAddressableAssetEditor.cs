...
public class DVAddressableAssetEditor : EditorWindow
{
    ...
    { 
        GetWindow<DVAddressableAssetEditor>("Addressable Asset Editor").Show();
    }
    ...
    private class DefaultAssetOption : EditorOption
    {
        ...
        {
            _assets = DVEditorUtil.DrawAssetList("Assets", ref _assetFieldFoldOut, _assets, false) as List<UnityEngine.Object>;
            ...
            if (_optionFoldout)
            {
                ...
                DVEditorUtil.SpaceHorizontalLayout(horizontalLayoutSpace, () =>
                {
                    string[] groupContents = DVAddressableUtil.GetGroupContents();

                    EditorGUILayout.LabelField("Addressable Group");
                    _optionGroupIndex = EditorGUILayout.Popup(_optionGroupIndex, groupContents);
                    _optionGroupName = groupContents[_optionGroupIndex].Replace(DVAddressableUtil.DEFAULT_GROUP_SIGN, "");
                });
        ...
        private void SetDefaultAsset()
        {
            ...
            for (int i = 0; i < _assets.Count; i++)
            {
                ...
                if (_optionFoldout)
                {
                    entry = DVAddressableUtil.GetOrAddEntry(guid, _optionGroupName, out bool isCreate);
                ...
                {
                    entry = DVAddressableUtil.GetOrAddEntry(guid, out bool isCreate);
                    addressableName = _assets[i].name.Replace(" ", "");
                }

                if (!string.IsNullOrEmpty(_optionAddressableLabel))
                    DVAddressableUtil.AddLabel(_optionAddressableLabel);

                DVAddressableUtil.SetupEntry(entry, addressableName, _optionAddressableLabel);
            }

            DVAddressableUtil.SaveSettings();
